...
// // import UpdateSearchFilter from "../components/UpdateSearchFilter";
// // import Breadcrumbs from "../components/Breadcrumbs";
// import { FaMapMarkerAlt, FaCheckCircle,
...
import UpdateSearchFilter from "../components/UpdateSearchFilter";
import Breadcrumbs from "../components/Breadcrumbs";
import { FaShareAlt, FaHeart, FaImages, FaMapMarkerAlt, FaCheckCircle, FaStar } from "react-icons/fa";
import Navbar from "./homePage/Navbar";
import Footer from "../components/2Footer";
import ReviewsModal from "../components/modals/ReviewModal";
...
//   const [activeTab, setActiveTab] = useState("Overview");
  ...
  const roomImages = new Array(5).fill("src/assets/images/StayImage.png");
  ...
  return (
    <div>
      ...
      <div className="mt-4 px-6 relative">
        ...
        <div className="grid grid-cols-2 gap-4 mt-2 p-10">
          <div className="relative">
            <img src={roomImages[0]} alt="Main Room" className="w-full h-[445px] object-cover rounded-lg" />
            <button className="absolute bottom-4 left-4 flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-md">
              <FaImages className="text-gray-600" /> Show all {roomImages.length} photos
            </button>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {roomImages.slice(1).map((image, index) => (
              <img key={index} src={image} alt={`Room ${index + 2}`} className="w-full h-[214px] object-cover rounded-lg" />
            ))}
          </div>
        </div>
      </div>
      ...
      {/* Overview Section */}
      <div className="w-[93%] mx-auto px-4 py-6 border-b border-gray-300">
        <h1 className="text-2xl font-bold text-black">Maison Fahrenheit Hotel</h1>
        <p className="text-gray-700 flex items-center gap-2 mt-2">
          <FaMapMarkerAlt className="text-gray-500" /> 80 Adetokunbo Ademola Street, Victoria Island, Lagos
        </p>
        <p className="text-green-600 flex items-center gap-2 mt-2">
          <FaCheckCircle /> Fully Refundable before Feb 9
        </p>
        <div className="flex items-center gap-2 mt-2">
          <span className="text-yellow-500 flex items-center gap-1">
            <FaStar /> 4.5
          ...
          <span className="text-gray-600">(180)</span>
          <button className="text-blue-600 underline cursor-pointer" onClick={() => setOpenModal(true)}>
